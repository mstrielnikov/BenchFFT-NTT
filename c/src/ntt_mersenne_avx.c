...
/*
 * Scalar 61-bit × 61-bit → 61-bit modular multiply.
 * We rely on __uint128_t to hold the full 122-bit intermediate product.
 */
static inline uint64_t m61_mul(uint64_t a, uint64_t b) {
    __uint128_t p = (__uint128_t)a * b;
    /* p < 2^122;  reduce mod 2^61-1 using the identity
     *   x mod (2^61-1) = (x_hi + x_lo) mod (2^61-1)
     * where x_hi = x >> 61, x_lo = x & (2^61-1). */
