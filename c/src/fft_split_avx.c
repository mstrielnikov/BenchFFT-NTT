...
static void fft_inplace_avx(double *re, double *im, size_t n, bool inverse) {
    ...
    for (size_t len = 2; len <= n; len <<= 1) {
        ...
        __m256d wlen_vec = _mm256_set_pd(wlen_im, wlen_re, wlen_im, wlen_re);
        ...
        for (size_t i = 0; i < n; i += len) {
            ...
            if (len >= 8) {
                for (; k + 4 <= len / 2; k += 4) {
                    ...
                    __m256d w_vec = _mm256_set_pd(w_im, w_re, w_im, w_re);
                    __m256d wlen_w = _mm256_mul_pd(w_vec, wlen_vec);
                    
                    __m256d v_real = _mm256_sub_pd(
                        _mm256_mul_pd(v_re, _mm256_unpacklo_pd(w_vec, w_vec)),
                        _mm256_mul_pd(v_im, _mm256_unpackhi_pd(w_vec, w_vec))
                    );
                    __m256d v_complex = _mm256_add_pd(
                        _mm256_mul_pd(v_re, _mm256_unpackhi_pd(w_vec, w_vec)),
                        _mm256_mul_pd(v_im, _mm256_unpacklo_pd(w_vec, w_vec))
                    );
