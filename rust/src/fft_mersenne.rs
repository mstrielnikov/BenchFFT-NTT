...
pub fn biguint_mul_fft_mersenne(a: &BigUInt, b: &BigUInt) -> BigUInt {
    ...
    for i in 0..n {
        let val = re_a[i] as u128 + carry;
        let word = val as u64;
        carry = val >> 64;

        result.push(m61_reduce(word as u128));
    }

    while carry > 0 {
        let word = carry as u64;
        result.push(m61_reduce(word as u128));
        carry >>= M61_P;
    }
...
mod tests {
    ...
    fn test_mul_small() {
        ...
        let c = biguint_mul_fft_mersenne(&a, &b);
